...
// ─── KUIS AI ───
document.getElementById("startQuizAiBtn").addEventListener("click", async () => {
  if (!currentUser) {
    alert("Login dulu ya!");
    return;
  }
  if (currentUser.energi <= 0) {
    alert("Energi habis! Tunggu reset besok.");
    return;
  }

  const kelas = document.getElementById("aiKelasSelect").value;
  const mapel = document.getElementById("aiMapelSelect").value;
  const topik = document.getElementById("aiTopikInput").value.trim();
  const jumlah = parseInt(document.getElementById("aiJumlahSelect").value);
  const durasi = parseInt(document.getElementById("aiTimerInput").value) || 120;

  // Validasi
  if (durasi < 30 || durasi > 1000) {
    alert("Durasi harus antara 30-1000 detik.");
    return;
  }
  if (!mapel) {
  ...
  try {
    const btn = document.getElementById("startQuizAiBtn");
    btn.disabled = true;
    btn.textContent = "⏳ Menghasilkan soal...";

    const response = await fetch(
      'https://yqbkkxibgsfaueaaezey.supabase.co/functions/v1/rag-chat/generate-soal',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mapel, kelas, jumlah, topik })
      }
    );
    ...
    sessionStorage.setItem('quizSoalAI', JSON.stringify(soal));
    sessionStorage.setItem('quizMode', 'ai');
    sessionStorage.setItem('quizMapel', mapel);
    sessionStorage.setItem('quizKelas', kelas);
    sessionStorage.setItem('quizTimer', durasi.toString());
    sessionStorage.setItem('quizJumlah', jumlah.toString());

    // Kurangi energi
    const { error } = await supabaseClient
      .from("user_progress")
      .update({ energi: currentUser.energi - 1 })
      .eq("username", currentUser.username);

    if (error) throw new Error('Gagal update energi');

    currentUser.energi -= 1;
    document.getElementById("energiDisplay").textContent = currentUser.energi;
    ...
    console.error(error);
    alert('Gagal generate soal AI: ' + error.message);
  } finally {
    const btn = document.getElementById("startQuizAiBtn");
    btn.disabled = false;
    btn.textContent = "🚀 Mulai Kuis AI (1 Energi)";
  }
});